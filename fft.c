...
void my_init()
{
	fft_r_data=(struct FFT_DATA *)malloc(sizeof(struct FFT_DATA)*Sample_Num);
	fft_g_data=(struct FFT_DATA *)malloc(sizeof(struct FFT_DATA)*Sample_Num);
	fft_b_data=(struct FFT_DATA *)malloc(sizeof(struct FFT_DATA)*Sample_Num);
...
int main(int argc,char *argv[])
{
	...
	Sample_Num=array_line_data;						//变换点的个数
	pixel_length=line_data*bmpHeight;				//图像包含像素点的总的个数

	/*图像的读入输出*/
	FILE *fp_out=fopen("fft_lena.bmp","wb+");		//图像的读入输出
	if(fp_out==NULL)
	{
		printf("输出图像打开失败\n");
		exit(-1);
	...
	//fwrite(fp_data,sizeof(char),pixel_length,fp_out);//输出为原图
	...
	{
		fft_r_data[i].data_R=fp_r_data[i];
		fft_r_data[i].data_I=0.0;
		fft_g_data[i].data_R=fp_g_data[i];
		fft_g_data[i].data_I=0.0;
		fft_b_data[i].data_R=fp_b_data[i];
		fft_b_data[i].data_I=0.0;
	}
