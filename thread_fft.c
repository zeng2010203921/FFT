...
void *thread_fun_r(unsigned char *fp_r_data)
{
...
void *thread_fun_g(unsigned char *fp_g_data)
{
...
void *thread_fun_b(unsigned char *fp_b_data)
{
...
int main(int argc,char *argv[])
{
	...
	/*图像的读入输出*/
	FILE *fp_out=fopen("thread_fft.bmp","wb+");		//图像的读入输出
	if(fp_out==NULL)
	{
		printf("输出图像打开失败\n");
		exit(-1);
	...
	pthread_t thread2;
	pthread_t thread3;
	if(pthread_create(&thread1,NULL,thread_fun_r,(void *)fp_r_data)!=0)
	{
		perror("pthread_create:");
		exit(-1);
	}

	if(pthread_create(&thread2,NULL,thread_fun_g,(void *)fp_g_data)!=0)
	{
		perror("pthread_create:");
		exit(-1);
	}

	if(pthread_create(&thread3,NULL,thread_fun_b,(void *)fp_b_data)!=0)
	{
		perror("pthread_create:");
		exit(-1);
	}
